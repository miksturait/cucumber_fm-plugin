...
It's support tool to cope wiht many features written in cucumber.

== Project is driven by two constrains:

* minimize manual work to lowest posible value
...
It's not even on alpha state - but i plan to relaese beta version in may (2010) - so it should be pretty soon usable
...
* crating feature
* editing content of feature

== Control Version System Integration

for non it only supports git

* every save trigger commit
* setting remote branch
* setting push policy
* triggering push to remote

== Report facility

=== table reports

* with posibility to export to spreadsheat

=== graph reports

* based on actual_data
* based on periodic data storing

=== Filter facility

* filter scenarios by tags
* have rules for categorizing tags
* accept logical AND, OR, NOT for building filter
